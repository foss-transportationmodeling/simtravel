###create class for deleting rows from the database.
...
class deleteDB:
	def __init__(self, dbobj, table_name, result, value):
		...
		try:	
			...
			self.result = self.dbobj.connection.execute("delete from %s where age = %s" % (self.table_name, self.value))
			
			#after deleting the data, print it
...
class TestDeleteDB(unittest.TestCase):
	...
	def testDB(self):
		...
		DBobj = deleteDB(self.dbobj, self.table_name, self.result, self.value)
...
#dispInstance = deleteDB('dbobj', 'users', 'result', '8')
