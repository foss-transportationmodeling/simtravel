...
class AbstractComponent(object):
    ...
    def pre_process(self, queryBrowser, subsample, 
                    ...
                    projectSkimsObject, householdStructureObject, db, fileLoc):
	...
	try:
        ...
        cacheColsTable = self.db.returnCols(self.writeToTable)

        t = time.time()
        convType = self.db.returnTypeConversion(self.writeToTable)
        dtypesInput = self.db.tableColTypes(self.writeToTable)
        data_to_write = self.data.columnsOfType(cacheColsTable, valid_data_filter, dtypesInput)
        print '\t\tConversion to appropriate record array took - %.4f' %(time.time()-t) 

	keyCols = tableNamesKeyDict[self.writeToTable][0]
	
	queryBrowser.copy_into_table(data_to_write.data, cacheColsTable, self.writeToTable, keyCols, fileLoc)	
        print '\t\tBatch Insert Took - %.4f' %(time.time()-t) 
