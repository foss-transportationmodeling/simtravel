...
#                self.import_schedule(filename)
    ...
    def checkTable(self, table):
        if self.new_obj.check_if_table_exists(str(table)):
            ...
            ret = QMessageBox.warning(self, "Import NHTS dataset...",
                                        QString("""Already, the database contains the NHTS dataset.\nDo you still want to import it?"""), 
    ...
    def import_person(self, filename):
        ...
        try:
                    ...
                    for name in headers:
                        ...
                        if lname == "houseid" or lname == "personid" or lname == "tdcaseid":
                            if lname == "houseid" or lname == "tdcaseid":
                                ssql = "%s%s %s NOT NULL,"%(ssql,lname,dtype)
                            else:
                                ssql = "%s%s %s NOT NULL,"%(ssql,lname,dtype)
                        else:
            ...
            QMessageBox.warning(self, "Import NHTS dataset...",
                        QString("""Error while inserting rows into the table.\nNull or text is not allowed."""),
                        QMessageBox.Yes)
            input.close()
            print e
...
#            o.write(header)
    ...
    #                            o.write(output)
                            ssql = 'INSERT INTO %s VALUES'%(table) + "('%s', '%s', '%s', '%d', '%d', '%d', '%d');" %(str(rownum),hid1,pid1,wto,stime,etime,duration)
                            self.new_obj.cursor.execute(ssql)
                            rownum += 1
                        ...
                        ssql = 'INSERT INTO %s VALUES'%(table) + "('%s', '%s', '%s', '%d', '%d', '%d', '%d');" %(str(rownum),hid2,pid2,wfrom,stime,etime,duration)
                        self.new_obj.cursor.execute(ssql)
    ...
    def open_folder(self):
        dialog = QFileDialog()
        temp = dialog.getOpenFileName(self,"Browse to select a csv file","","Comma Delimit (*.csv)")
