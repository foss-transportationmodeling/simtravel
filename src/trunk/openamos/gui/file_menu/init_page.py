...
class InitPage(QWizardPage):
    def __init__(self, parent=None):
        ...
        self.connect(self.pronameline, SIGNAL("textEdited(const QString&)"), self.nameCheck)
        self.connect(self.proloccombobox, SIGNAL("activated(int)"), self.proloccombobox.browseFolder)
        self.connect(self.proloccombobox, SIGNAL("currentIndexChanged(int)"), self.locationCheck)
