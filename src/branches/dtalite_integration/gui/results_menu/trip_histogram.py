...
class Absreldiff(Matplot):
    ...
    def __init__(self, project, parent=None):
        ...
        if self.isValid():
            self.valid = True
            self.setWindowTitle(
                "Average Absolute Relative Difference Distribution")
            ...
            QMessageBox.warning(
                self, "Results", "A table with name - performance_statistics does not exist.", QMessageBox.Ok)
    ...
    def on_draw(self):
        ...
        if self.retrieveResults():
            ...
            self.axes.bar(
                self.err1, self.err2, align='center', width=10, color='b')
    ...
    def retrieveResults(self):
        ...
        try:
            retrieve = self.executeSelectQuery(
                cursor, vars, tablename, filter, group)
