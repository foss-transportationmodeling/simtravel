...
#        addfilterlayout.addWidget(segment,0,1)
        ...
        self.connect(
            self.dialogButtonBox, SIGNAL("accepted()"), self.disconnects)
    ...
    def executeSelectQuery(self, cursor, vars, tablename, filter="", group="", order=""):
        ...
        try:
            ...
            if filter != "" and group != "" and order != "":
                cursor.execute("""SELECT %s FROM %s WHERE %s GROUP BY %s ORDER BY %s""" % (
                    vars, tablename, filter, group, order))
                temp = cursor.fetchall()
            elif filter != "" and group == "":
                cursor.execute("""SELECT %s FROM %s WHERE %s ORDER BY %s""" % (
                    vars, tablename, filter, order))
                temp = cursor.fetchall()
            elif filter == "" and group != "":
                cursor.execute("""SELECT %s FROM %s GROUP BY %s ORDER BY %s""" % (
                    vars, tablename, group, order))
                temp = cursor.fetchall()
            else:
                cursor.execute(
                    """SELECT %s FROM %s ORDER BY %s""" % (vars, tablename, order))
                temp = cursor.fetchall()
...
#            return False
    ...
    def fill_variable1(self, tablename):
        ...
        if tablename == "trips_r":
            temp = ["starttime", "endtime", "tripmode",
...
class LabComboBox(QWidget):
    ...
    def __init__(self, label, list, parent=None):
        ...
        self.connect(self.combobox, SIGNAL(
            "currentIndexChanged(const QString&)"), self.emitSignal)
