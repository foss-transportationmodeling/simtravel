...
class Read_Shape(QDialog):
    ...
    def __init__(self, configobject=None, parent=None):
        ...
        self.connect(
            self.shapebutton, SIGNAL("clicked(bool)"), self.open_folder)
        self.connect(
            self.dialogButtonBox, SIGNAL("accepted()"), self.create_sql)
        self.connect(
            self.dialogButtonBox, SIGNAL("rejected()"), self.disconnects)
    ...
    def open_folder(self):
        ...
        temp = dialog.getOpenFileName(
            self, "Browse to select a project configuration file", "", "Shape File (*.shp)")
    ...
    def create_sql(self):
        ...
        if shpname != '' and sid != '' and pid != '':
            ...
            QMessageBox.information(self, "",
                                    QString(
                                        """Shapefile importing is successful!"""),
                                    QMessageBox.Yes)
        else:
            QMessageBox.warning(self, "Import spatial information...",
                                QString(
                                    """You should select shapefile and enter EPSG code."""),
                                QMessageBox.Yes)
    ...
    def create_state(self, f, sf):
        f.write(
            u'CREATE TABLE "%s" (gid serial PRIMARY KEY,\n' % (self.tablename))
