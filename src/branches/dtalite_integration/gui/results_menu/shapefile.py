...
class Reader:
    ...
    def __shape(self):
        ...
        if shapeType in (13, 15, 18, 31):
            ...
            record.z = _Array(
                'd', unpack("<%sd" % nPoints, f.read(nPoints * 8)))
    ...
    def __dbfHeaderLength(self):
        ...
        if not self.__dbfHdrLength:
            ...
            (self.numRecords, self.__dbfHdrLength) = unpack(
                "<xxxxLH22x", f.read(32))
        ...
        for s in self._shapes:
            ...
            if s.shapeType in (3, 5, 8, 13, 15, 18, 23, 25, 28, 31):
                try:
                    ...
                    raise ShapefileException(
                        "Falied to write bounding box for record %s. Expected floats." % recNum)
            # Shape types with parts
            ...
            if s.shapeType in (3, 5, 8, 13, 15, 23, 25, 31):
                for part in s.parts:
                    try:
                        ...
                        raise ShapefileException(
                            "Failed to write points for record %s. Expected floats." % recNum)
            ...
            if s.shapeType in (9, 13, 15, 18, 23, 25, 31):
                try:
                    ...
                    raise ShapefileException(
                        "Failed to write measure extremes for record %s. Expected floats" % recNum)
                for part in s.parts:
