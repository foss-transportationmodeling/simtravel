...
class DatabaseConfig(QDialog):
    ...
    def __init__(self, configobject=None, parent=None):
        ...
        self.connect(
            self.dialogButtonBox, SIGNAL("accepted()"), self.storeDatabase)
        self.connect(
            self.dialogButtonBox, SIGNAL("rejected()"), SLOT("reject()"))

        self.configobject = configobject
        self.protocolline.setText(
            self.configobject.getConfigElement(DB_CONFIG, DB_PROTOCOL))
        self.hostline.setText(
            self.configobject.getConfigElement(DB_CONFIG, DB_HOST))
        self.dbnameline.setText(
            self.configobject.getConfigElement(DB_CONFIG, DB_NAME))
        self.usernameline.setText(
            self.configobject.getConfigElement(DB_CONFIG, DB_USER))
        self.passwdline.setText(
            self.configobject.getConfigElement(DB_CONFIG, DB_PASS))
