...
class Travel_Reconciliation_System(QWidget):
    ...
    def __init__(self, parent=None, co=None):
        ...
        pattern_onciliation_button = QPushButton(
            'Activity-travel \nPattern Reconciliation', self)
        pattern_onciliation_button.setGeometry(
            (size.width()) / 2 - 100, size.height() / 2 - 350, 200, 80)
        ...
        person_constraints_2_button = QPushButton(
            "Within person constraints", self)
        person_constraints_2_button.setGeometry(
            (size.width()) / 2 - 100, size.height() / 2 - 240, 200, 80)
        ...
        hhold_constraints_button = QPushButton(
            "Within household constraints", self)
        hhold_constraints_button.setGeometry(
            (size.width()) / 2 - 100, size.height() / 2 - 130, 200, 80)
        ...
        adjustment_2_button = QPushButton(
            'Duration adjustment \nafter arrival', self)
        adjustment_2_button.setGeometry(
            (size.width()) / 2 - 100, size.height() / 2 - 20, 200, 80)
...
#        diag.exec_()
    ...
    def paintEvent(self, parent=None):
        ...
        line.drawLine(widgetwidth / 2, widgetheight / 2 - 270,
                      widgetwidth / 2, widgetheight / 2 - 20)
        line.end()
        # Drawing arrow
        arrow = QPainter()
        arrow.begin(self)
