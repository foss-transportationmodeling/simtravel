...
class VehicleOwnershipModels(QWidget):
    ...
    def __init__(self, parent=None, co=None):
        ...
        self.num_vehs_button = QPushButton(
            'Count of Vehicles \nOwned by the Household', self)
        self.num_vehs_button.setGeometry(
            (size.width()) / 2 - 100, size.height() / 2 - 330, 200, 50)
        self.num_vehs_button.setStyleSheet(
            self.color.isUserModel(MODELKEY_NUMVEHS))
        ...
        self.veh_types_button = QPushButton(
            "Vehicle Body Type \nfor each Household Vehicle", self)
        self.veh_types_button.setGeometry(
            (size.width()) / 2 - 100, size.height() / 2 - 250, 200, 200)
        self.veh_types_button.setStyleSheet(
            self.color.isUserModel(MODELKEY_VEHTYPE))
        self.connect(
            self.veh_types_button, SIGNAL('clicked()'), self.veh_types)
    ...
    def paintEvent(self, parent=None):
        ...
        line.setPen(pen)
        line.drawLine(widgetwidth / 2, widgetheight / 2 - 280,
                      widgetwidth / 2, widgetheight / 2 - 250)
        line.end()
        # Drawing arrow
        arrow = QPainter()
        arrow.begin(self)
