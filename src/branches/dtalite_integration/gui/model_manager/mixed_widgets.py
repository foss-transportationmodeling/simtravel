...
class AbstractMixedWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.connect(self.tableswidget, SIGNAL(
            "itemClicked (QListWidgetItem *)"), self.populateColumns)
        self.connect(
            self.addbutton, SIGNAL("clicked(bool)"), self.addAttribute)
        self.connect(
            self.delbutton, SIGNAL("clicked(bool)"), self.delAttribute)
    ...
    def addAttribute(self):
        ...
        if (name != "") & (value != ""):
            ...
            self.attritable.setItem(
                self.attritable.rowCount() - 1, 0, tableitem)
