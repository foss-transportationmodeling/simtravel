...
class AbstractModWidget(QGroupBox):
    ...
    def makeProbChoiceWidget(self):
        ...
        self.connect(
            self.choiceaddbutton, SIGNAL("clicked(bool)"), self.addChoice)
        self.connect(
            self.choicedelbutton, SIGNAL("clicked(bool)"), self.delChoice)
...
#        parentdialog = self.parent()
        # parentdialog.tablelist)
        self.tableswidget.addItems(self.fatherdialog.tablelist)
        ...
        self.varstable.setHorizontalHeaderLabels(
            ['Table', 'Column', 'Coefficient'])
        ...
        self.connect(self.tableswidget, SIGNAL(
            "itemClicked (QListWidgetItem *)"), self.populateFields)
        self.connect(
            self.varsbutton, SIGNAL("clicked(bool)"), self.addVariable)
...
#        parentdialog = self.parent()
        if seltab != "runtime":
            # parentdialog.coldict[seltab])
            self.colswidget.addItems(self.fatherdialog.coldict[seltab])
    ...
    def makeNestWidget(self):
        ...
        self.connect(
            self.nesteaddbutton, SIGNAL("clicked(bool)"), self.addNest)
    ...
    def makeVarianceWidget(self):
        ...
        self.mainlayout.addWidget(
            self.variancevwidget, 0, 0, 1, 1, Qt.AlignLeft)
...
class CountModWidget(AbstractModWidget):
    ...
    def makeCountTypeWidget(self):
        ...
        self.mainlayout.addWidget(
            self.counttypewidget, 0, 0, 1, 1, Qt.AlignLeft)
...
class OrderedModWidget(AbstractModWidget):
    ...
    def makeOrdChoiceWidget(self, x=0, y=0):
        ...
        self.choicetable.setHorizontalHeaderLabels(
            ['Alternative', 'Value', 'Threshold'])
        self.choicelayout.addWidget(self.choicetable)

        self.mainlayout.addWidget(self.choicewidget, x, y, 1, 1)

        self.connect(
            self.choiceaddbutton, SIGNAL("clicked(bool)"), self.addOrdChoice)
        self.connect(
            self.choicedelbutton, SIGNAL("clicked(bool)"), self.delOrdChoice)
...
class MNLogitModWidget(AbstractModWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.makeChoiceWidget(1, 0)
        self.makeVarsWidget(1, 1)
        self.connect(self.choicetable, SIGNAL(
            "currentCellChanged (int,int,int,int)"), self.showVarsTable)
...
class NLogitModWidget(AbstractModWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.connect(self.choicetable, SIGNAL(
            "currentItemChanged (QTableWidgetItem *,QTableWidgetItem *)"), self.showVarsTable)
...
class SchduleModWidget(AbstractModWidget):
    ...
    def __init__(self, parent=None):
        ...
        varspec1 = [["HouseholdIdName", "", ""], ["PersonIdName", "", ""], ["ScheduleIdName", "", ""], ["ActivityTypeName", "", ""],
                    ["LocationIdName", "", ""], ["StartTimeName", "", ""], [
                        "EndTimeName", "", ""], ["DurationName", "", ""],
                    ["DependentPersonName", "", ""]]
        ...
        varspec7 = [["TripDependentPersonIdName", "", ""], ["TripCountName1", "", ""], [
            "TripCountName", "", ""], ["ActDependentPersonIdName", "", ""]]
        self.specs["PersonsArrivedAttributes"] = varspec7

        self.connect(self.choicelist, SIGNAL(
            "currentItemChanged (QListWidgetItem *,QListWidgetItem *)"), self.showVarsTable)
