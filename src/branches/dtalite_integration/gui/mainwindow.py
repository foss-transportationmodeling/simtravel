...
class MainWindow(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        # Variable for a project properties; can be used to see if a project is
        # open or not
        self.proconfig = None
...
#        self.centralwidgetscroll.setWidget(self.centralwidget)
        ...
        # Setting help_dock_widget on the bottom of window to show instruction
        # message
        ...
        self.connect(self.model_management, SIGNAL(
            'itemClicked (QTreeWidgetItem *,int)'), self.showflowchart)
        ...
        up_action = self.createaction(
            "", self.getup, "", "arrow_up", "Move up an element in the tree.")
        down_action = self.createaction(
            "", self.getdown, "", "arrow_down", "Move down an element in the tree.")
        tools.addAction(up_action)
        tools.addAction(down_action)
        buttonlayout.addWidget(tools)

        # Adding model_management and data_management to
        # all_manager_dock_widget
        ...
        # Method to disable menus and graphics based on whether a project is
        # open or not
        self.checkProject()
...
#                                            "import", "Import data.")
        ...
        self.addActions(
            self.data_menu, (data_export_action, data_dataconfig_action,))
...
#        self.addActions(self.run_menu, (setting_preference_action, ))
    ...
    # Defining Results
        ...
        self.addActions(self.result_menu, (activity_pattern_action,
                                           person_schedule_action, kml_tirps_action, export_output, ))
    ...
    # Defining help
        ...
        self.addActions(
            self.help_menu, (help_documentation_action, None, help_about_action,))
...
# Show flowcharts from model management tree widget
    def showflowchart(self, selitem, col):
        ...
        # selitem.text(col) == COMPMODEL_SCHSTATUS or selitem.text(col) ==
        # COMPMODEL_CHIDDEPEND or selitem.text(col) == COMPMODEL_PERATTR:
        if selitem.text(col) == COMPMODEL_AFTSCHACTIVITY:
            # self.models.show_child_model()
        ...
        # COMP_ACTTRAVRECONCILIATION:
        if selitem.text(col) == COMPMODEL_RECONCILSTRTADJ:
            self.models.show_travel_reconciliation_system()
...
#            self.models = Models(self.centralwidget)
            ...
            self.proconfig = ConfigObject(
                configfileloc=str(self.project_open.file))
            self.checkProject()
            self.data_menu.actions()[2].setEnabled(True)
            self.run_menu.actions()[0].setEnabled(True)
            self.result_menu.actions()[0].setEnabled(True)
    ...
    def projectSaveAs(self):
        file = QFileDialog.getSaveFileName(self, QString("Save As..."),
                                           "%s" % self.proconfig.getConfigElement(
                                               PROJECT_HOME),
                                           "XML File (*.xml)")
...
#                self.setWindowTitle("OpenAMOS: Version-1.0 (%s)" %self.project.name)
    ...
    def checkProject(self):
        ...
        if actpro:
            self.setWindowTitle(
                "OpenAMOS: Version-1.0 (%s)" % self.proconfig.getConfigElement(PROJECT, NAME))
    ...
    def results_schedules(self):
        if self.proconfig <> None:
            # MakePlot(self.proconfig,"schedule_final_r")
