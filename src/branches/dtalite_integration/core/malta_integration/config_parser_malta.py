...
class ConfigParser(object):
        ...
        for compElement in self.iterator:
            if compElement.get('name') == component_name:
                ...
                if analysisInterval is not None:
                    analysisIntervalElement = compElement.find(
                        'AnalysisInterval')
                    if analysisIntervalElement is None:
                        continue
                    # print 'OLD ANALYSIS INTERVAL START _ ',
                    # analysisIntervalElement.get('start')
                    analysisIntervalElement.set(
                        'start', str(analysisInterval + 1))
    ...
    def parse_models(self, projectSeed=0):
        ...
        for element in self.configObject.iter(tag=etree.Element):
            if element.tag == 'Component':
                ...
                componentList += self.parse_analysis_interval_and_create_component(
                    element, projectSeed)
                ...
                for i in range(endInterval - startInterval):
                    for subElement in element.getiterator('SubComponent'):
                        # print 'Found component in component list - ',
                        # subElement.get('name')
    ...
    def parse_analysis_interval_and_create_component(self, component_element, projectSeed, analysisInterval=None):
        ti = time.time()
        comp_name, comp_read_table, comp_write_table, comp_write_to_table2 = self.return_component_attribs(
            component_element)
        ...
        if interval_element is not None:
            ...
            for i in range(endInterval - startInterval):
                ...
                tempComponent = self.create_component(
                    component_element, projectSeed, analysisInterval)
        ...
        # print '\t\tTime taken to parse across all analysis intervals %.4f'
        # %(time.time()-ti)
    ...
    def create_component(self, component_element, projectSeed=0, analysisInterval=None):
        ...
        analysisIntervalFilter_element = component_element.find(
            'AnalysisIntervalFilter')
        if analysisIntervalFilter_element is not None:
            analysisIntervalFilter = self.return_table_var(
                analysisIntervalFilter_element)
            analysisIntervalCondition = analysisIntervalFilter_element.get(
                "condition")
        ...
        spatialConstIterator = component_element.getiterator(
            "SpatialConstraints")
        spatialConst_list = []
        ...
        dynamicSpatialConstIterator = component_element.getiterator(
        ...
        if preProcessData_element is not None:
            pre_run_filter = self.return_filter_condition_list(
                preProcessData_element)
        ...
        if consistencyChecks_element is not None:
            post_run_filter = self.return_filter_condition_list(
                consistencyChecks_element)
        else:
        ...
        for model in self.model_list:
            # print model.dep_varname, 'Interaction model - ',
            # isinstance(model.model,
            # openamos.core.models.interaction_model.InteractionModel)
    ...
    def return_history_info(self, history_element):
        ...
        historyInfoObject = HistoryInfo(
            histTableName, histAggVar, histVarAggConditions)
    ...
    def create_model_object(self, model_element, projectSeed=0, analysisInterval=None):
        ...
        if model_formulation == 'Multinomial Logit':
            if model_element.find('AlternativeSet') is None:
                self.create_multinomial_logit_object(
                    model_element, projectSeed)
            else:
                self.create_multinomial_logit_object_generic_locs(
                    model_element, projectSeed)
        ...
        if model_formulation == 'Clean Fixed Activity Schedule':
            self.create_clean_fixed_activity_schedule(
                model_element, projectSeed)

        if model_formulation == 'Clean Aggregate Activity Schedule':
            self.create_clean_aggregate_activity_schedule(
                model_element, projectSeed)

        if model_formulation == 'Child Dependency Allocation Terminal':
            self.create_child_dependency_allocation_object(
                model_element, terminal=True, projectSeed=projectSeed)

        if model_formulation == 'Child Dependency':
            self.create_child_dependency_allocation_object(
                model_element, terminal=False, projectSeed=projectSeed)
        ...
        if model_formulation == 'Evolution Post Process':
            self.create_evolution_post_process_object(
                model_element, projectSeed)

        if model_formulation == 'Emigration':
            self.create_migration_object(
                model_element, projectSeed, migrationType='Emigration')

        if model_formulation == 'Immigration':
            self.create_migration_object(
                model_element, projectSeed, migrationType='Immigration', analysisInterval=analysisInterval)

        if model_formulation == 'Trip Occupant Processing':
            self.create_trip_occupant_processing_object(
                model_element, projectSeed)

        if model_formulation == 'Persons On Trip Arrival Processing':
            self.create_persons_arrived_processing_object(
                model_element, projectSeed)
    ...
    def create_multinomial_logit_object_generic_locs(self, model_element, projectSeed=0):
        ...
        coeff_list, vars_list = self.return_coeff_vars(
            model_element, alternativeSet)
    ...
    def create_linear_object_for_locations(self, model_element, spatialConst_list, projectSeed=0):
        ...
        for const in spatialConst_list:
            if const.countChoices is not None:
                ...
                # Filter set
                run_filter_set_element = model_element.find(
                    'RunUntilConditionSet')
                if run_filter_set_element is not None:
                    run_filter_type = run_filter_set_element.get('type')
                else:
                    run_filter_type = None
    ...
    def create_reconcile_schedules_object(self, model_element, projectSeed=0):
        ...
        activityAttribsSpec = self.return_activity_attribs(
            activity_attribs_element)
    ...
    def create_clean_fixed_activity_schedule(self, model_element, projectSeed=0):
        ...
        dailyStatusAttribsSpec = self.return_daily_status_attribs(
            dailystatus_attribs_element)

        dependency_attribs_element = model_element.find('Dependency')
        dependencyAttribsSpec = self.return_dependency_attribs(
            dependency_attribs_element)
    ...
    def create_evolution_post_process_object(self, model_element, projectSeed):
        ...
        evolutionAttribsSpec = self.return_evolution_attribs(
            evolution_attribs_element)
    ...
    def create_unique_records_object(self, model_element, projectSeed):
        ...
        uniqueRecordsVarNameParsed = self.return_table_var(
            unique_records_element)

        variable_list.append(uniqueRecordsVarNameParsed)

        specification = UniqueRecordsSpecification(
            uniqueRecordsVarNameParsed[1])
    ...
    def create_persons_arrived_processing_object(self, model_element, projectSeed):
        ...
        pers_arrived_attribs_element = model_element.find(
            'PersonsArrivedAttributes')
        persArrivedAttribSpec = self.return_arrived_pers_attribs(
            pers_arrived_attribs_element)
    ...
    def create_trip_occupant_processing_object(self, model_element, projectSeed):
        ...
        trip_dep_pers_attribs_element = model_element.find(
            'TripDependentPersonAttributes')
        tripDepAttribSpec = self.return_dep_pers_attribs(
            trip_dep_pers_attribs_element)
    ...
    def create_schedule_adjustment_object(self, model_element):
        ...
        if arrival_info_element is not None:
            arrivalInfoAttribsSpec = self.return_arrival_info_attribs(
                arrival_info_element)
        else:
        ...
        if occupancy_info_element is not None:
            occupancyInfoAttribsSpec = self.return_occupancy_info_attribs(
                occupancy_info_element)
        else:
    ...
    def return_arrived_pers_attribs(self, trip_arrived_pers_attribs_element):
        ...
        trpDepPersIdName_element = trip_arrived_pers_attribs_element.find(
            'TripDependentPersonIdName')
        trpDepPersIdNameParsed = self.return_table_var(
            trpDepPersIdName_element)
        variable_list.append(trpDepPersIdNameParsed)

        tripCountName_element = trip_arrived_pers_attribs_element.find(
            'TripCountName')
        tripCountNameParsed = self.return_table_var(tripCountName_element)
        variable_list.append(tripCountNameParsed)

        endTripCountName_element = trip_arrived_pers_attribs_element.find(
            'EndTripCountName')
        if endTripCountName_element is not None:
            endTripCountNameParsed = self.return_table_var(
                endTripCountName_element)
            variable_list.append(endTripCountNameParsed)
        else:
            endTripCountNameParsed = (None, None)

        actDepPersIdName_element = trip_arrived_pers_attribs_element.find(
            'ActDependentPersonIdName')
        actDepPersIdNameParsed = self.return_table_var(
            actDepPersIdName_element)
    ...
    def return_dep_pers_attribs(self, trip_dep_pers_attribs_element):
        ...
        tripPurpFrom_element = trip_dep_pers_attribs_element.find(
            'TripPurposeFrom')
        tripPurpFromParsed = self.return_table_var(tripPurpFrom_element)
        variable_list.append(tripPurpFromParsed)

        trpDepPersIdName_element = trip_dep_pers_attribs_element.find(
            'TripDependentPersonIdName')
        trpDepPersIdNameParsed = self.return_table_var(
            trpDepPersIdName_element)
        variable_list.append(trpDepPersIdNameParsed)

        lastTrpActDepPersIdName_element = trip_dep_pers_attribs_element.find(
            'LastTripDependentPersonIdName')
        lastTrpActDepPersIdNameParsed = self.return_table_var(
            lastTrpActDepPersIdName_element)
        variable_list.append(lastTrpActDepPersIdNameParsed)

        stActDepPersIdName_element = trip_dep_pers_attribs_element.find(
            'StActDependentPersonIdName')
        stActDepPersIdNameParsed = self.return_table_var(
            stActDepPersIdName_element)
        variable_list.append(stActDepPersIdNameParsed)

        enActDepPersIdName_element = trip_dep_pers_attribs_element.find(
            'EnActDependentPersonIdName')
        enActDepPersIdNameParsed = self.return_table_var(
            enActDepPersIdName_element)
        variable_list.append(enActDepPersIdNameParsed)

        stActTripCountName_element = trip_dep_pers_attribs_element.find(
            'StActTripCountName')
        stActTripCountNameParsed = self.return_table_var(
            stActTripCountName_element)
        variable_list.append(stActTripCountNameParsed)

        enActTripCountName_element = trip_dep_pers_attribs_element.find(
            'EnActTripCountName')
        enActTripCountNameParsed = self.return_table_var(
            enActTripCountName_element)
        variable_list.append(enActTripCountNameParsed)

        tripCountName_element = trip_dep_pers_attribs_element.find(
            'TripCountName')
        tripCountNameParsed = self.return_table_var(tripCountName_element)
        variable_list.append(tripCountNameParsed)

        lastTripCountName_element = trip_dep_pers_attribs_element.find(
            'LastTripCountName')
        lastTripCountNameParsed = self.return_table_var(
            lastTripCountName_element)
        variable_list.append(lastTripCountNameParsed)

        persOnNetworkName_element = trip_dep_pers_attribs_element.find(
            'PersonOnNetwork')
        if persOnNetworkName_element is not None:
            persOnNetworkNameParsed = self.return_table_var(
                persOnNetworkName_element)
        ...
        tripDepAttribSpec = TripDependentPersonAttributes(tripPurpFromParsed[1],
                                                          trpDepPersIdNameParsed[
                                                              1],
                                                          lastTrpActDepPersIdNameParsed[
                                                              1],
                                                          stActDepPersIdNameParsed[
                                                              1],
                                                          enActDepPersIdNameParsed[
                                                              1],
                                                          persOnNetworkNameParsed[
                                                              1],
                                                          tripCountNameParsed[
                                                              1],
                                                          lastTripCountNameParsed[
                                                              1],
                                                          stActTripCountNameParsed[
                                                              1],
    ...
    def return_daily_status_attribs(self, dailystatus_attribs_element):
        ...
        dailySchStatus_element = dailystatus_attribs_element.find(
            'DailySchoolStatus')
        dailySchStatusParsed = self.return_table_var(dailySchStatus_element)
        variable_list.append(dailySchStatusParsed)

        dailyWrkStatus_element = dailystatus_attribs_element.find(
            'DailyWorkStatus')
    ...
    def return_arrival_info_attribs(self, arrival_info_element):
        ...
        dependentperson_element = arrival_info_element.find(
            'DependentPersonId')
    ...
    def return_dependency_attribs(self, dependency_attribs_element):
        ...
        childDependency_element = dependency_attribs_element.find(
            'ChildDependency')
        ...
        dependencySpec = DependencyAttribsSpecification(
            childDependencyParsed[1], elderlyDependencyName=None)
    ...
    def return_person_attribs(self, person_attribs_element):
        ...
        maritalStatusName_element = person_attribs_element.find(
            'MaritalStatus')
        ...
        hispanicIndicatorName_element = person_attribs_element.find(
            'HispanicIndicator')
        hispanicIndicatorParsed = self.return_table_var(
            hispanicIndicatorName_element)
    ...
    def return_evolution_attribs(self, evolution_attribs_element):
        ...
        mortalityName_element = evolution_attribs_element.find(
            'MortalityStatus')
        ...
        educationInYearsName_element = evolution_attribs_element.find(
            'EducationInYears')
        educationInYearsParsed = self.return_table_var(
            educationInYearsName_element)
        variable_list.append(educationInYearsParsed)

        residenceDecisionName_element = evolution_attribs_element.find(
            'ResidenceDecision')
        residenceDecisionParsed = self.return_table_var(
            residenceDecisionName_element)
        variable_list.append(residenceDecisionParsed)

        laborParticipationName_element = evolution_attribs_element.find(
            'LaborParticipation')
        laborParticipationParsed = self.return_table_var(
            laborParticipationName_element)
        ...
        marriageDecisionName_element = evolution_attribs_element.find(
            'MarriageDecision')
        marriageDecisionParsed = self.return_table_var(
            marriageDecisionName_element)
        variable_list.append(marriageDecisionParsed)

        divorceDecisionName_element = evolution_attribs_element.find(
            'DivorceDecision')
        divorceDecisionParsed = self.return_table_var(
            divorceDecisionName_element)
        ...
        evolutionAttribsSpec = EvolutionAttributesSpecification(mortalityParsed[1],
                                                                ...
                                                                educationParsed[
                                                                    1],
                                                                educationInYearsParsed[
                                                                    1],
                                                                residenceDecisionParsed[
                                                                    1],
                                                                laborParticipationParsed[
                                                                    1],
                                                                occupationParsed[
                                                                    1],
                                                                incomeParsed[
                                                                    1],
                                                                marriageDecisionParsed[
                                                                    1],
    ...
    def return_activity_attribs(self, activity_attribs_element):
        ...
        householdIdName_element = activity_attribs_element.find(
            'HouseholdIdName')
        ...
        scheduleIdName_element = activity_attribs_element.find(
            'ScheduleIdName')
        scheduleIdParsed = self.return_table_var(scheduleIdName_element)
        variable_list.append(scheduleIdParsed)

        activityTypeName_element = activity_attribs_element.find(
            'ActivityTypeName')
        ...
        locationIdName_element = activity_attribs_element.find(
            'LocationIdName')
        ...
        dependentPersonName_element = activity_attribs_element.find(
            'DependentPersonName')
        dependentPersonParsed = self.return_table_var(
            dependentPersonName_element)
        variable_list.append(dependentPersonParsed)
    ...
    def return_coeff_vars(self, element, alternativeSet=None):
        ...
        for i in variableIterator:
            dep_var, inverse_dict = self.check_for_interaction_terms(
                i, alternativeSet)
    ...
    def return_spatial_query(self, spatial_query_element):
        ...
        if activity_element is not None:
            activityTypeFilter = self.return_activity_type_condition(
                activity_element)
        else:
            activityTypeFilter = None

        startConstraint_element = spatial_query_element.find('Start')
        startConstraint = self.return_spatio_temporal_constraint(
            startConstraint_element)
        startTableVar = self.return_table_var(startConstraint_element)

        endConstraint_element = spatial_query_element.find('End')
        endConstraint = self.return_spatio_temporal_constraint(
            endConstraint_element)
        endTableVar = self.return_table_var(endConstraint_element)

        votdConstraint_element = spatial_query_element.find(
            'ValueofTimePerDistance')
        ...
        locationIdVar = None
        locationInformationElement = spatial_query_element.find(
        ...
        if locationInformationElement is not None:
            ...
            locationVarsIterator = locationInformationElement.getiterator(
                'LocationVariable')
        ...
        if locationInformationElement is not None:
            locationFilterList = self.return_filter_condition_list(
                locationInformationElement)
            locationFilterTypeElement = locationInformationElement.find(
                'FilterSet')
    ...
    def return_spatio_temporal_constraint(self, constraint_element):
        ...
        constraint = SpatioTemporalConstraint(
            table, location_field, time_field)
        return constraint
    ...
    def dummy(self):
        ...
        if filterValue is not None:
            ...
            self.component_variable_list = (self.component_variable_list +
                                            variable_list_val)
        runUntilFilter = DataFilter(
            varname_ind, runUntilCondition, filterValue)
...
if __name__ == '__main__':
    ...
    query_gen, cols = newobject.fetch_selected_rows(
        pers_class_name, 'employ', '1')
    ...
    query_gen, cols = newobject.select_join(
        temp_dict, 'household_id', pers_class_name, 'employ', '1')
