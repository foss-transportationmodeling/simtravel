...
class SimulationManager(object):
    ...
    def parse_config(self):
        ...
        for comp in self.componentList:
            for mod in comp.model_list:
                self.modSeedDict[
                    (comp.component_name, mod.dep_varname)] = copy.deepcopy(mod.seed)
    ...
    def run_selected_components_for_dtalite(self, analysisInterval, tripInfoArrivals=array([]), filename='open_amos_trip_min'):
        try:
            tripInfo = self.run_components(
                analysisInterval, tripInfoArrivals, filename)
        except Exception, e:
            print 'Error occurred: ', e
            traceback.print_exc(file=sys.stdout)
            raw_input()
            raise Exception, 'Error occurred when executing component - %s: %s' % (
                comp.component_name, e)
        return tripInfo
        ...
        for comp in self.componentList:
            ...
            for loop in range(loop_num):
                ...
                if comp.component_name == 'ArrivalTimeInformation':
                    ...
                    # Reset seed
                    for mod in comp.model_list:
                        mod.seed = self.modSeedDict[
                            (comp.component_name, mod.dep_varname)] + analysisInterval - 1
                ...
                if data is not None:
                    ...
                    if comp.component_name == "ExtractAllTravelEpisodes":
                        tripInfo = comp.run(
                            data, self.queryBrowser, self.skimsMatrix, self.uniqueIds, fileLoc)
                        ...
                        # print 'Destination Zone Ids After Mod - ',
                        # tripInfo[:5,6]
                        self.add_header_for_dtalite(
                            fileLoc, analysisInterval, tripInfo, filename)
                        # raw_input()
                    else:
                        comp.run(
                            data, self.queryBrowser, self.skimsMatrix, self.uniqueIds, fileLoc)
                    ...
                    tripInfo = zeros((1, 18))
                    self.create_csv_to_dtalite(
                        fileLoc, analysisInterval, filename)
                    """
                    tripInfo = zeros((3,11))
    ...
    def load_skims_matrix(self, comp, ttTableLocation, distTableLocation):
        ...
        # self.temp_remove_outof_zone_id(ttTableLocation) # Just temporarily
        # and delete it after the presentation on 02/11/2014
    ...
    def load_currentskims_matrix(self, ttTableLocation, distTableLocation=None):
        ...
        if self.loadCurrentSkim:
            print 'Real Time tt Table Location - ', ttTableLocation
            print 'Real Time dist Table Location - ', distTableLocation
