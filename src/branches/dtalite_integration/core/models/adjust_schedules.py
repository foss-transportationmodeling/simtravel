...
class AdjustSchedules(Model):
    ...
    def check_hhldIndices_for_multiple_person_arrivals(self):
        numPersons = self.hhldIndicesOfPersons[
            :, -1] - self.hhldIndicesOfPersons[:, -2]
    ...
    def create_col_numbers(self, colNamesDict):
        ...
        if self.schedAdjType == "Arrival Adjustment":
            self.tripDependentPersonCol = colNamesDict[
                self.arrivalInfoAttribs.dependentPersonName]
            self.actualArrivalCol = colNamesDict[
                self.arrivalInfoAttribs.actualArrivalName]
            self.expectedArrivalCol = colNamesDict[
                self.arrivalInfoAttribs.expectedArrivalName]

        if self.schedAdjType == "Occupancy Adjustment":
            self.tripIndicatorCol = colNamesDict[
                self.occupancyInfoAttribs.tripIndicatorName]
            self.tripStTimeCol = colNamesDict[
                self.occupancyInfoAttribs.startTimeName]
    ...
    def return_arrival_info(self, schedulesForPerson):
        ...
        tripDependentPerson = schedulesForPerson[
            0, self.tripDependentPersonCol]
...
class TestReconcileModel(unittest.TestCase):
    ...
    def setUp(self):
        self.data = genfromtxt(
            "/home/karthik/simtravel/test/mag_zone_dynamic/schedule_txt_small.csv", delimiter=",", dtype=int)
