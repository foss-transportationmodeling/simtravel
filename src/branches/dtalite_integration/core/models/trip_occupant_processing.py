...
class TripOccupantProcessing(Model):
    ...
    def create_col_numbers(self, colnamesDict):
        ...
        self.tripPurposeFromNameCol = colnamesDict[
            self.tripDepAttribSpec.tripPpurposeFromName]
        self.tripDepNameCol = colnamesDict[self.tripDepAttribSpec.tripDepName]
        self.lastTripDepNameCol = colnamesDict[
            self.tripDepAttribSpec.lastTripDepName]
        self.stActDepNameCol = colnamesDict[
            self.tripDepAttribSpec.stActDepName]
        self.enActDepNameCol = colnamesDict[
            self.tripDepAttribSpec.enActDepName]
        self.personOnNetworkNameCol = colnamesDict[
            self.tripDepAttribSpec.personOnNetworkName]
        self.tripCountCol = colnamesDict[self.tripDepAttribSpec.tripCountName]

        self.stActTripCountCol = colnamesDict[
            self.tripDepAttribSpec.stActTripCountName]
        self.enActTripCountCol = colnamesDict[
            self.tripDepAttribSpec.enActTripCountName]
        self.lastTripCountCol = colnamesDict[
            self.tripDepAttribSpec.lastTripCountName]
    ...
    def resolve_consistency(self, data, seed):
        ...
        for rowId in range(data.rows):
            ...
            if self.personOnNetworkNameCol <> None:
                ...
                # THIS IS LIKE AN OVERRIDE FOR PROCESSING OCCUPANCY WHEN THE
                # PERSON IS STILL ON THE NETWORK FOR THE DYNAMIC CASE
                if self.personOnNetwork == 1:
                    # print '\ta.3. Person is still on the network therefore
                    # the trip dependent person is set to the old occupancy -
                    # ', lastTripDep
            ...
            if tripPurposeFrom == 600 or tripPurposeFrom == 601:
                if stActDep > 100:
                    # print '\ta.2. The occupancy of the trip changed and
                    # processing occupancy now ... stActDep - %s' %stActDep
                    ...
                    # print '\t-->New tripcount dep pers - %s'
                    # %tripCountDepPers

                else:
                    # print '\ta.2. No need to process occupancy this is a
                    # dependent person ... stActDep - %s' %stActDep
                    tripDepPers = stActDep
                    tripCountDepPers = stActTripCount
            else:
                # print '\ta.1. The occupancy of the trip did not change and
                # occupancy is - ', lastTripDep
                tripDepPers = lastTripDep
                tripCountDepPers = lastTripCount
    ...
    def parse_trip_dependentpersonid_count(self, hid, pid,
        ...
        # print '\t\tThe number of people on the previous trip is - %s and they
        # are - %s and depid is - %s' %(len(lastTripPersons), lastTripPersons,
        # lastTripDep)
        ...
        if tripPurposeFrom == 600:
            ...
            # print '\t\tThis is a pickup so we should take all people from
            # dummy pickup loc; count of people - %s and they are - %s ... '
            # %(len(pickupPersons), pickupPersons)
        ...
        if tripPurposeFrom == 601:
            ...
            # print '\t\tThis is a dropoff so we should take all people from
            # dummy pickup loc; count of people - %s and they are - %s ... '
            # %(len(dropoffPersons), dropoffPersons)
            ...
            for person in dropoffPersons:
                try:
                    ...
                    raise Exception, 'The hid - %s person - %s should be on the trip check again error occurred and last trip persons - %s' % (
                        hid, person, lastTripPersons)
