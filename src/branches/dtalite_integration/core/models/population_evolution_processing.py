...
class PopulationEvolutionProcessing(Model):
    ...
    def create_col_numbers(self, colnamesDict):
        ...
        self.enrollment_fCol = colnamesDict[
            self.evolutionAttribs.enrollment_fName]
        ...
        self.educInYears_fCol = colnamesDict[
            self.evolutionAttribs.educInYears_fName]
        self.residenceType_fCol = colnamesDict[
            self.evolutionAttribs.residenceType_fName]
        self.laborParticipation_fCol = colnamesDict[
            self.evolutionAttribs.laborParticipation_fName]
        self.occupation_fCol = colnamesDict[
            self.evolutionAttribs.occupation_fName]
        self.income_fCol = colnamesDict[self.evolutionAttribs.income_fName]
        self.marriageDecision_fCol = colnamesDict[
            self.evolutionAttribs.marriageDecision_fName]
        self.divorceDecision_fCol = colnamesDict[
            self.evolutionAttribs.divorceDecision_fName]
    ...
    def resolve_consistency(self, data, seed):
        ...
        for hhldIndex in self.hhldIndicesOfPersons:
            ...
            personRecs = DataArray(
                data.data[firstPersonRec:lastPersonRec, :], data.varnames)
            ...
            for personRec in personRecs.data:
                personObject = Person(
                    personRec[self.hidCol], personRec[self.pidCol])
                ...
                personObject.set_evolution_attributes(personRec[self.morality_fCol],
                                                      ...
                                                      personRec[
                                                          self.enrollment_fCol],
                                                      personRec[
                                                          self.grade_fCol],
                                                      personRec[
                                                          self.educ_fCol],
                                                      personRec[
                                                          self.educInYears_fCol],
                                                      personRec[
                                                          self.residenceType_fCol],
                                                      personRec[
                                                          self.laborParticipation_fCol],
                                                      personRec[
                                                          self.occupation_fCol],
                                                      personRec[
                                                          self.income_fCol],
                                                      personRec[
                                                          self.marriageDecision_fCol],
                                                      personRec[self.divorceDecision_fCol])

                householdObject.add_person(personObject)
            additionalHouseholdObject, highestHid = householdObject.evolve_population(
                highestHid)
