...
class DivideData(object):
    ...
    def get_table_info(self):
        ...
        # input tables
        self.house_cols, self.house_dt, self.house_def, self.house_keys = self.get_tab_structure(
            self.house_tab)
        self.persons_cols, self.persons_dt, self.persons_def, self.persons_keys = self.get_tab_structure(
            self.persons_tab)

        # output tables
        self.child_cols, self.child_dt, self.child_def, self.child_keys = self.get_tab_structure(
            self.child_tab)
        self.school_cols, self.school_dt, self.school_def, self.school_keys = self.get_tab_structure(
            self.school_tab)
        self.work_cols, self.work_dt, self.work_def, self.work_keys = self.get_tab_structure(
            self.work_tab)
        self.veh_count_cols, self.veh_count_dt, self.veh_count_def, self.veh_count_keys = self.get_tab_structure(
            self.veh_count_tab)
        self.ltrec_cols, self.ltrec_dt, self.ltrec_def, self.ltrec_keys = self.get_tab_structure(
            self.ltrec_tab)
        self.schedule_cols, self.schedule_dt, self.schedule_def, self.schedule_keys = self.get_tab_structure(
            self.schedule_tab)
        self.trips_cols, self.trips_dt, self.trips_def, self.trips_keys = self.get_tab_structure(
            self.trips_tab)
        self.veh_cols, self.veh_dt, self.veh_def, self.veh_keys = self.get_tab_structure(
            self.veh_tab)
        self.workers_cols, self.workers_dt, self.workers_def, self.workers_keys = self.get_tab_structure(
            self.workers_tab)
        self.schedule_child_cols, self.schedule_child_dt, self.schedule_child_def, self.schedule_child_keys = self.get_tab_structure(
            self.schedule_child_tab)
        self.schedule_clean_cols, self.schedule_clean_dt, self.schedule_clean_def, self.schedule_clean_keys = self.get_tab_structure(
            self.schedule_clean_tab)
        ...
        self.schedule_daily_cols, self.schedule_daily_dt, self.schedule_daily_def, self.schedule_daily_keys = self.get_tab_structure(
            self.schedule_daily_tab)
        self.schedule_final_cols, self.schedule_final_dt, self.schedule_final_def, self.schedule_final_keys = self.get_tab_structure(
            self.schedule_final_tab)
        self.schedule_intravel_cols, self.schedule_intravel_dt, self.schedule_intravel_def, self.schedule_intravel_keys = self.get_tab_structure(
            self.schedule_intravel_tab)
    ...
    def create_partitions(self, databasename, parts):
        ...
        for each in range(parts):
            ...
            self.dbcon_obj.create_table(
                self.house_tab, self.house_cols, self.house_dt, self.house_def, self.house_keys)
            self.dbcon_obj.create_table(
                self.persons_tab, self.persons_cols, self.persons_dt, self.persons_def, self.persons_keys)
            self.dbcon_obj.create_table(
                self.child_tab, self.child_cols, self.child_dt, self.child_def, self.child_keys)
            self.dbcon_obj.create_table(
                self.school_tab, self.school_cols, self.school_dt, self.school_def, self.school_keys)
            self.dbcon_obj.create_table(
                self.work_tab, self.work_cols, self.work_dt, self.work_def, self.work_keys)
            self.dbcon_obj.create_table(
                self.veh_count_tab, self.veh_count_cols, self.veh_count_dt, self.veh_count_def, self.veh_count_keys)
            self.dbcon_obj.create_table(
                self.ltrec_tab, self.ltrec_cols, self.ltrec_dt, self.ltrec_def, self.ltrec_keys)
            self.dbcon_obj.create_table(
                self.schedule_tab, self.schedule_cols, self.schedule_dt, self.schedule_def, self.schedule_keys)
            self.dbcon_obj.create_table(
                self.trips_tab, self.trips_cols, self.trips_dt, self.trips_def, self.trips_keys)
            self.dbcon_obj.create_table(
                self.veh_tab, self.veh_cols, self.veh_dt, self.veh_def, self.veh_keys)
            self.dbcon_obj.create_table(
                self.workers_tab, self.workers_cols, self.workers_dt, self.workers_def, self.workers_keys)
    ...
    def get_interval_rows(self, databasename, each_table, column_name, interval_list, index):
        ...
        sql_string = "SELECT * FROM %s WHERE %s BETWEEN %s AND %s" % (
            each_table, column_name, low, high)
    ...
    def delete_records(self, databasename, table1, table2, column_name, index, interval_list):
        ...
        sql_string1 = "DELETE FROM %s WHERE %s NOT BETWEEN %s AND %s" % (
            table1, column_name, low, high)
    ...
    def partition_data(self, parts, table_list,
        ...
        for index in range(parts):
            ...
            for each_table in table_list:
                ...
                table_rows = self.get_interval_rows(
                    self.database_name, each_table, column_name, interval_list, index)
                print '---> Data selected based on the houseid'

                # insert the selected data into the required tables
                insert = self.insert_data(
                    self.database_name, table_rows, each_table, index)
