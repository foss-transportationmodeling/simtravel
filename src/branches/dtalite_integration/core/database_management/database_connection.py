...
class DataBaseConnection(object):
    ...
    def check_if_database_exists(self):
        ...
        if self.database_config_object.protocol is 'postgres':
            connect_string = '%s://%s:%s@%s:5432' % (
                self.protocol, self.user_name, self.password, self.host_name)
            ...
            self.connection = engine.text(
                "select datname from pg_database").execute()
            ...
            if self.database_config_object.protocol is 'postgres':
                try:
                    ...
                    engine.raw_connection().set_isolation_level(
                        extensions.ISOLATION_LEVEL_AUTOCOMMIT)
                    self.connection = engine.text(
                        "create database %s encoding = 'utf8'" % self.database_config_object.database_name).execute()
        ...
        if db_flag:
            try:
                ...
                engine.raw_connection().set_isolation_level(
                    extensions.ISOLATION_LEVEL_AUTOCOMMIT)
                self.connection = engine.text(
                    "drop database %s" % self.database_config_object.database_name).execute()
                engine.dispose()
                self.connection.close()
    ...
    def define_mapping(self, ctype, map_flag=True):
        ...
        # Before proceeding with the mapping check if mapping or reverse
        # mapping
        if map_flag:
            # mapping datatyes
    ...
    def new_connection(self):
        ...
        try:
            ...
            self.sessionInstance = sessionmaker(
                bind=self.engine, autoflush=True, autocommit=True)
            #self.session = Session()
        except Exception, e:
    ...
    def get_table_desc(self, columns, ctypes, keys):
        ...
        for col, ctype, ktype in zip(col_names, col_types, key_types):
            ...
            if ktype == "1":
                column = Column(
                    col, self.define_mapping(ctype), primary_key=True)
            else:
...
class TestDBConfiguration(unittest.TestCase):
    ...
    def testDB(self):
        ...
        new_obj = DataBaseConnection(
            self.protocol, self.user_name, self.password, self.host_name, self.database_name, self.database_config_object)
