...
class Household(object):
    ...
    def return_person_list(self):
        ...
        for personId in self.persons.keys():
            ...
            if person.race1 == 0:
                raw_input(
                    'Print the race of the person: hid- %s and pid - %s is 0' % (self.hid, person.pid))
    ...
    def set_household_attributes(self, bldgsz, hht, hinc, noc,
        ...
        # self.persons = persons (No need to assign these; they are updated as
        # person objects are added)
        self.unittype = unittype
        self.vehicl = vehicl
        # self.wif = wif (No need to assign these; they are updated as person
        # objects are added)
    ...
    def evolve_population(self, highestHid):
        ...
        for personId in personIds:
            person = self.persons[personId]
            # print '\tRelationship of all people - %s and gender - %s'
            # %(self.relateDict[person.relate], self.sexDict[person.sex])
    ...
    def process_divorce(self):
        ...
        if hhldrId <> False:
            hhldr = self.persons[hhldrId]
            # changing the marriage status of the hhldr
            hhldr.marstat = 3

        if partnerId <> False:
            partner = self.persons[partnerId]
            # changing the marriage status of the partner
            partner.marstat = 3
            # changing the relationship status to householder
            partner.relate = 1
        ...
        if hhldrId and partnerId:
            ...
            for personId in personIds:
                ...
                # Stepson/stepdaughter
                    ...
                    partnersFamily.append(personId)
                    # updating the relationship status to Father/mother
                    person.relate = 7
                # Brother-in-law/Sister-in-law
                elif person.relate == 12:
                    partnersFamily.append(personId)
                    # updating the relationship status to Father/mother
                    person.relate = 6
            ...
            vehiclOriHhld = self.rndGen.return_random_integers(
                0, self.vehicl, 1)[0]
    ...
    def process_household_type_after_divorce(self):
        ...
        if hhldr.sex == 1 and len(self.persons) == 1:
            ...
            # Non family household: Female householder living alone
            self.hht = 6
    ...
    def clean_schedules_for_in_home_episodes(self, seed):
        ...
        for pid in self.personIds:
            ...
            if not person._check_for_conflicts():
                ...
                raise Exception, 'The person still has conflicts - %s, %s' % (
                    self.hid, pid)
    ...
    def clean_schedules(self, seed):
        ...
        # print '\tPerson Ids with no fixed activities - ',
        # self.noDailyFixedActPersonIds
   ...
   """
    ...
    def adjust_schedules_given_arrival_info(self, seed, personarrivedId=None):
        ...
        # print 'number of persons for this household that arrived including
        # dependents - %s and they are pids - %s' %(len(self.persons),
        # self.persons.keys())
        ...
        for pid in self.persons.keys():
            ...
            if (personarrived.child_dependency == 1 or
                ...
                # print '\t    the schedule for this person need not be
                # modified; this person is not independent and can only be
                # adjusted due to schedule adjustment of adult'
                continue
        ...
        for pid in indepPidsForAdj:
            ...
            # print 'FOR INDEP PERSON BEFORE - ',
            # personarrived.print_activity_list()
            self.adjust_person_schedules_given_arrival_info(
                personarrived, seed)

            # print 'FOR INDEP PERSON AFTER',
            # personarrived.print_activity_list()

            # print '\t    --Dependentpersonid for trip- ', personarrived.tripDependentPerson
            # print '\t    --Dependentpersonid for next act- ',
            # personarrived.destAct.dependentPersonId
            ...
            # print '\t     --schedules for more than one person need to be
            # adjusted and these persons are - ', adjOtherPersonIds
    ...
    def traverse_through_dependency_chain(self, arrivaltime):
        ...
        # for personAct in self.affectedActs.keys():
            if personAct.dependentPersonId > 0:
                ...
                adjOtherPersons = self.parse_personids(
                    personAct.dependentPersonId)

                for depPersonId in adjOtherPersons:
                    depPerson = self.persons[depPersonId]
                    print '\t\tfor dependent person - ', depPersonId

                    depPersonConflicts = depPerson._identify_match_activities(
                        [personAct])
                    self.add_anchor_activity(
                        depPerson, depPersonConflicts[0], arrivaltime)
    ...
    def resolve_conflict(self, person, refAct):
        ...
        for act in conflictActs:
            ...
            # the conflict act overlaps with the tail end of the reference
            # activity
    ...
    def adjust_person_schedules_given_arrival_info(self, person, seed=1):
        ...
        # print '\tactual arrival - %s and expected arrival - %s'
        # %(person.actualArrival, person.expectedArrival)
        ...
        if person.destAct.startTime > person.actualArrival:
            # print '\n-- Arrived earlier than expected; moving destination
            # activity to earlier --'
            self.push_destination_activity_to_earlier(person)

        if person.destAct.startTime < person.actualArrival:
            # print '\n-- Arrived later than expected; adjustment for
            # activities needs to happen here --'
    ...
    def push_destination_activity_to_earlier(self, person):
        ...
        if person.destAct.actType == 100 or person.destAct.actType == 600:
            self.add_anchor_activity(
                person, person.destAct, person.actualArrival)
    ...
    def wait_push_subsequent_activities(self, person):
        ...
        # print 'before adjustment for occupancy ----',
    ...
    def adjust_push_subsequent_activities(self, person, refArrivalTime=None):
        ...
        # because if there is a missed activity without adding an anchor the
        # subsequent activity is starting from nowhere and hence the
        # inconsistency ...
        self.add_anchor_activity(person, person.destAct, actualArrival)
        pushedActs = True

        # Adjusting when more than one activity is affected: Push all
        # activities
        for act in person.expectedActivities:
            # if (act.endTime < actualArrival and act.dependentPersonId > 0 and
            # (act.actType == 600 or act.actType == 601)):
        ...
        if len(actsToBeAdjusted) > 2:
            ...
            if (
                ...
                # subsequent act is child dependent act
                ((nextAct.actType >= 450 and nextAct.actType <= 490) or (nextAct.actType == 151)) and
                # the dropoff is within duration for subsequent act
                ((nextAct.startTime < actualArrival)
                 and (nextAct.endTime > actualArrival))
            ):
        ...
        for act in actsToBeAdjusted[index:]:
            ...
            # if actEnd-act.startTime >= 0:
            if actEnd - act.startTime >= 0:
                # print '-->This ', act, ' is being moved by ',
    ...
    def identify_joint_episodes(self):
        ...
        for pid in self.dependencyPersonIds:
            ...
            for indepPersonId in indepPersonActivityList.keys():
                ...
                indepPersonActs = indepPerson._identify_conflict_activities(
                    activityList)
                print
                check = False
                for act in indepPersonActs:
                    if act.dependentPersonId == 99:
                        ...
                        act.dependentPersonId = act.dependentPersonId * \
                            100. + pid
                        check = True
    ...
    def lineup_activities_based_on_activities(self, seed):
        ...
        if len(self.dependencyPersonIds) == 0:
            # print 'DEPENDENCIES DO NOT EXIST; ACTIVITIES NEED NOT BE
            # ALLOCATED TO INDEPENDENT PERSONS'
            return
        ...
        for pid in depAct.keys():
            ...
            for i in range(len(depActs)):
                ...
                if depenAct.startTime <> indepAct.startTime and depenAct.endTime <> indepAct.endTime:
                    ...
                    if depenAct.actType == 600:
                        depPerson.move_start_end(
                            depenAct, indepAct.startTime - depenAct.startTime - 1)
                    ...
                    if depenAct.actType >= 400 and depenAct.actType <= 450:
                        depPerson.move_start_end_by_diff_values(
                            depenAct, indepAct.startTime, indepAct.endTime)
            ...
            if len(depIHActs) <> len(indepIHActs):
                ...
                """
                raw_input(
                    'Number of IH Acts is not the same i.e. count dependent IH episodes <> count dependent IH episodes allocated to independent persons')
            ...
            for i in range(len(depIHActs)):
                ...
                if depenIHAct.actType == 101 and indepIHAct.actType == 151:
                    ...
                    depPerson.move_start_end_by_diff_values(
                        depenIHAct, indepIHAct.startTime, indepIHAct.endTime)
                ...
                if depenIHAct.actType == 101 and indepIHAct.actType == 100:
                    if depenIHAct.startTime < indepIHAct.startTime and indepIHAct.startTime:
                        ...
                        depPerson.move_start_end(
                            depenIHAct, indepIHAct.startTime - depenIHAct.startTime)
    ...
    def lineup_activities(self, seed):
        ...
        for pid in self.indepPersonIds:
            ...
            for i in range(len(person.listOfActivityEpisodes)):
                ...
                if act.actType >= 450 and act.actType <= 500:
                    # print 'Other OH joint activity with dependent child
                    # identified'
                    depPersonIds = self.parse_personids(act.dependentPersonId)
                    for depPid in depPersonIds:
                        if depPid in otherOHDepActs:
                            otherOHDepActs[depPid].append(
                                (act.startTime, act.endTime))
                        else:
                            otherOHDepActs[depPid] = [
                                (act.startTime, act.endTime)]
        ...
        for depPid in depTripsSt.keys():
            ...
            try:
                ...
                # print 'No OH Acts to be adjusted for this dependent person -
                # ', depPid
                pass
            ...
            for i in range(len(depPerson.listOfActivityEpisodes)):
                ...
                if act.actType == 600:
                    if firstTripSt == -1:
                        # print 'start anchor', act.startTime, firstTripSt ==
                        # -1
            ...
            # adjusting the vertices based on arrivals and departure pegs
            # derived from the independent adjults
            depPersonTempList = []
            lastTripEn = 0
            ...
            for i in range(len(depPerson.listOfActivityEpisodes)):
                ...
                if stAct.actType == 600 and enAct.actType == 601 and stAct.dependentPersonId <> 99:
                    ...
                    if st == lastTripEn + 1:
                        ...
                        # if the trip is very short then move the reference end
                        # of the trip too
                        if en < st:
                            en = st + 1
                    ...
                    if stAct.endTime <> st:
                        depPerson.move_start_end(
                            stAct, st - stAct.endTime - 1, removeAdd=False)
                        # print '\t\tMod St Vertex', stAct

                    # print '\t    En Vertex - ', enAct
                    if enAct.startTime <> en + 1:
                        depPerson.move_start_end(
                            enAct, en - enAct.startTime - 1, removeAdd=False)
                ...
                # All waiting associated for trips with non household members
                # why do you have to move pickup and dropoff simultaneously?
                if stAct.actType == 600 and stAct.dependentPersonId == 99:
                    ...
                    if st == lastTripEn:
                        ...
                        #raw_input('the waiting and discretizing error case for trips with nonhh members')
                    """
                    # print 'pickup non hh end', stAct.endTime, ' proposed end
                    # -', st
                ...
                if enAct.actType >= 400 and enAct.actType <= 450 and enAct.dependentPersonId <> 99:
                    ...
                    if enAct.endTime <> ohActEn or enAct.startTime <> ohActSt:
                        depPerson.move_start_end_by_diff_values(
                            enAct, ohActSt, ohActEn, removeAdd=False)
                    ohActCount += 1
            ...
            if lastActBeforeFirstPickup.endTime < firstTripSt - 1:
                depPerson.move_end(
                    lastActBeforeFirstPickup, firstTripSt - 1, removeAdd=False)
            ...
            for i in range(len(depPersonTripAnchorsList) / 2):
                ...
                if (actVertEn.startTime - actVertSt.endTime) <= 1:
                    # print '\tThis activity was not pursued ... the person
                    # visited the destination and moved on ... '
                    pass

                else:
                    depPerson.move_start_end_by_diff_values(
                        actBet, actVertSt.endTime, actVertEn.startTime, removeAdd=False)
    ...
    def extract_tours(self, person):
        ...
        for i in range(len(person.listOfActivityEpisodes)):
            ...
            if enAct.location == -99:
                ...
                # print 'act - ', enAct, 'stopCount-', stops, 'occuList-',
                # occuList, 'Mean occupancy - ',
                # (array(occuList).sum())/float(array(occuList).size)
                ...
                if (stAct.actType == 601 and
                        ((enAct.actType >= 450 and enAct.actType <= 500) or
                         ...
                         (stAct.location == enAct.location and enAct.actType == 101) or
                                 ...
                                 (person.child_dependency ==
                                  1 and enAct.actType <= 101)
                         )
                    ):
                    stAct.actType = 651
    ...
    def fill_gaps_in_dependent_person_schedule(self, person):
        ...
        for i in range(len(person.listOfActivityEpisodes)):
            ...
            if enAct.startTime < stAct.endTime and enAct.actType == 100:
                ...
                person.move_start_end(
                    enAct, stAct.endTime - enAct.startTime, removeAdd=False)
                tempList.append((stAct.startTime, stAct))
                stAct = enAct
                continue
            ...
            if stAct.startTime < enAct.endTime and stAct.actType == 100 and stAct.startTime <> 0:
                print 'ending sojourn'
                person.move_start_end(
                    stAct, enAct.endTime - stAct.startTime, removeAdd=False)
                tempList.append((stAct.startTime, stAct))
                stAct = enAct
                continue
        ...
        for pid in self.indepPersonIds:
            ...
            for i in range(len(acts)):
                ...
                if (stAct.actType == nextAct.actType and stAct.actType == 600 and
                    ...
                    # Getting the actual start activity and not the copy for
                    # indep person
                    stAct = person._identify_match_activity(stAct)[0]
                    # Getting actual next activity and not the copy for indep
                    # person
                    nextAct = person._identify_match_activity(nextAct)[0]

                    nextTwoActs = person._identify_next_activity(
                        nextAct, count=2)
                    ...
                    print '\t\tPickup', stAct
                    stActDepPers = self.parse_personids(
                        stAct.dependentPersonId)
                    print '\t\tDropof', stActCombo

                    print '\n\t\tPickup', nextAct
                    nextActDepPers = self.parse_personids(
                        nextAct.dependentPersonId)
                    print '\t\tDropof', nextActCombo
                    nextToNextAct = person._identify_next_activity(
                        nextActCombo)[0]
                    ...
                    for depPersId in nextActDepPers:
                        # The pickup dependentperson id updated to include
                        # everyone
                        stAct.dependentPersonId = stAct.dependentPersonId * \
                            100. + depPersId
                    ...
                    for depPersId in stActDepPers:
                        ...
                        prevToPickupSt = depPers._identify_previous_activity(
                            stAct)[0]
                        # Getting the actual activities and not the copy for
                        # dep person
                        pickUpDropOffActsSt = depPers._identify_match_activities(
                            [stAct, stActCombo])
                        nextToDropOffSt = depPers._identify_next_activity(
                            stActCombo)[0]
                    ...
                    for depPersId in nextActDepPers:
                        print '\n\tFor dependent person - ', depPersId
                        depPers = self.persons[depPersId]
                        prevToPickup = depPers._identify_previous_activity(
                            nextActCopy)[0]
                        # Getting the actual activities and not the copy for
                        # dep person
                        pickUpDropOffActs = depPers._identify_match_activities(
                            [nextActCopy, nextActComboCopy])
                        nextToDropOff = depPers._identify_next_activity(
                            nextActComboCopy)[0]
                        ...
                        depPers.move_end(
                            prevToPickup, prevToPickup.endTime - moveBy + 1)
                        # since we moved we can allocate this persons
                        prevToPickup.dependentPersonId = prevToPickupSt.dependentPersonId
                        # act before pickup to the same adult as the one we
                        # used as reference to move i.e. st
                        prevDepAdult = self.persons[
                            prevToPickupSt.dependentPersonId]
                        self.print_activity_list(prevDepAdult)
                        prevDepAdultPrevToPickup = prevDepAdult._identify_match_activities(
                            [prevToPickupSt])[0]
                        prevDepAdultPrevToPickup.dependentPersonId = prevDepAdultPrevToPickup.dependentPersonId * \
                            100. + \
                            depPersId  # also update the corresponding act dependency for indep adult

                        depPers.move_start_end(pickUpDropOffActs[0], -moveBy)
                        if moveConflictCreated:
                            depPers.move_start_end(
                                pickUpDropOffActs[1], -moveBy + 1)
                        else:
                            depPers.move_start_end(
                                pickUpDropOffActs[1], -moveBy)

                        nextToDropOffForIndep = person._identify_match_activities(
                            [nextToDropOff])
                        ...
                        if moveConflictCreated:
                            depPers.move_start(
                                nextToDropOff, nextToDropOff.startTime - moveBy + 1 + 1)
                            if nextToDropOffForIndep <> []:
                                depPers.move_start(
                                    nextToDropOffForIndep, nextToDropOff.startTime - moveBy + 1 + 1)
                        else:
                            depPers.move_start(
                                nextToDropOff, nextToDropOff.startTime - moveBy + 1)
                            if nextToDropOffForIndep <> []:
                                depPers.move_start(
                                    nextToDropOffForIndep, nextToDropOff.startTime - moveBy + 1)
                        ...
                        # MOVEBY START AND END - moves start and end of
                        # activity by that amount
                    break

                # for each person in the dependent person list now move/adjust
                # the pickup, dropoff and the nexst activity
    ...
    def parse_personids(self, tripDep):
        ...
        # print tripDep, pers
        if len(pers) > 1:
            # print '\t\tExciting picking up more than one
            # persoallocate_dependent_activitiesn ... '
            pass
        return pers
    ...
    def locked_adults_for_person(self, person):
        ...
        for i in range(len(acts)):
            ...
            if act.dependentPersonId <> 0:
                ...
                self.depenLocks[depPers][indepPers][
                    stTime:endTime] = 1  # dependent person tree
    ...
    def allocate_terminal_dependent_activities(self, seed):
        ...
        if len(self.dependencyPersonIds) == 0:
            ...
            for pid in self.dependencyPersonIds:
                ...
                # print '\n\t\t1.2. End of Day/Start of Day: Someone needs to
                # be there'
                endAct = person.lastEpisode
                # print '\t\t', endAct
                self.adjust_terminal_activity_episodes(
                    pid, endAct, start=False)
        ...
        if len(self.dependencyPersonIds) == 0:
            ...
            for pid in self.dependencyPersonIds:
                ...
                while (len(actsOfDepPerson) > 0):
                    ...
                    # if (endAct.location <> stAct.location) and
                    # (endAct.actType >= 400 and endAct.actType < 500):
                    ...
                    # first check to see when same location and if end is
                    # maintenance if so then allocate the end act
                    ...
                    # if above is not true then no need to allocate the end
                    # activity ... when both acts are oh episodes
                    ...
                    # if above is not true then no need to allocate the end
                    # activity ... when first act is ih and second act is oh
                    if (stAct.location == endAct.location) and (stAct.actType < 200 and endAct.actType >= 200):
                        self.allocate_pickup_dropoff(pid, stAct, endAct)
                        stAct = endAct
                        #raw_input('assigned pickup dropoff for acts with same loc but discretionary/other activity engagement')
                    ...
                    # if above is not true then allocate the end activity ...
                    # first act is oh and end act is ih
                    ...
                    # if above is not true then allocate the end activity ...
                    # first act is oh and end act is sojourn
                    ...
                    # ALLOCATE PICK-UP DROP-OFF TO END SOJOURN ADULT WHEN
                    # PREVIOUS OUTHOME - Case2 (see below started coding)
    ...
    def adjust_terminal_activity_episodes(self, depPersonId, actOfdepPerson, start=True):
        ...
        # print '\t\t\t\tAdjusting the terminal start-"%s" activity episodes to
        # ensure that the child is taken care of' %(start)
        depPerson = self.persons[depPersonId]
        ...
        for pid in self.noDailyFixedActPersonIds:
            ...
            if start:
                check = person.check_start_of_day(
                    actOfdepPerson.endTime, depPersonId)
            else:
                check = person.check_end_of_day(
                    actOfdepPerson.startTime, depPersonId)

            if check:
                # print '\t\t\t\tPerson with no fixed activities found and id
                # is -- ', pid
                self.update_depPersonId_for_terminal_episode(
                    start, depPersonId, pid)
                return True
        ...
        for pid in self.dailyFixedActPersonIds:
            ...
            if check:
                # print '\t\t\t\tPerson with fixed activities found and id is
                # -- ', pid
                self.update_depPersonId_for_terminal_episode(
                    start, depPersonId, pid)
                return True
        ...
        if len(self.noDailyFixedActPersonIds) > 0:
            self.rndGen.shuffle_sequence(self.noDailyFixedActPersonIds)
            # check to see if there any people without fixed acts and
            pid = self.noDailyFixedActPersonIds[0]
        ...
        if pid is None:
            self.update_depPersonId_for_terminal_episode(
                start, depPersonId, 99)
            # print "Exception, --There are no independent adults in the
            # household for hid - %s--" %(self.hid)
            return

        # print "\t\t\t\t--Randomly independent adults in the household is
        # selected and id is --", pid

        person = self.persons[pid]

        # print '----before - ', person.lastEpisode.dependentPersonId
        if start:
            depPerson.move_start_of_day(
                person.firstEpisode.endTime, pid, dependent=True)
            if person.firstEpisode.dependentPersonId == 0:
                person.firstEpisode.dependentPersonId = 100 + depPersonId
            else:
                person.firstEpisode.dependentPersonId = 100. * \
                    person.firstEpisode.dependentPersonId + depPersonId
        else:
            depPerson.move_end_of_day(
                person.lastEpisode.startTime, pid, dependent=True)

            if person.lastEpisode.dependentPersonId == 0:
                person.lastEpisode.dependentPersonId = 100 + depPersonId
            else:
                person.lastEpisode.dependentPersonId = 100. * \
                    person.lastEpisode.dependentPersonId + depPersonId
    ...
    def allocate_ih_activity(self, depPersonId, act):
        ...
        for pid in self.indepPersonIds:
            ...
            if person._check_for_ih_conflicts(act, depPersonId):
                # print '\t\t\t\tPerson - %s is already home so he is allocated
                # this activity' %(pid)
        ...
        # print " \t\t\t--- > Exception: No person identified; not possible;
        # the dependent acts cause conflicts < --- "
        ...
        # print "\t\t\t\t--Random independent adults in the household is
        # selected and id is --", pid
    ...
    def allocate_pickup_dropoff(self, depPersonId, stAct, endAct):
        ...
        dummyActPickUp, dummyActDropOff = self.create_dummy_activity(
            depPersonId, stAct, endAct)

        dummyActPickUp.dependentPersonId = 100 + depPersonId
        dummyActDropOff.dependentPersonId = 100 + depPersonId

        # Person without fixed activities
        # We allocate pickup/dropoff to the first person with no fixed
        # activities that we find
        ...
        for pid in self.noDailyFixedActPersonIds:
            ...
            if not person._check_for_conflicts_with_activity([dummyActPickUp, dummyActDropOff]):
                if person._check_for_home_to_home_trips():
                    ...
                    person.remove_episodes([dummyActPickUp, dummyActDropOff])
                    confActs = person._identify_conflict_activities(
                        [dummyActPickUp, dummyActDropOff])

                    if len(confActs) > 0:
                        # If confActs == 0 then it probably means that a home
                        # to home trip was being introduced
                        jointPickUpDropOffFlag = self.check_conflicts_with_pickup_dropoff(
                            confActs, dummyActPickUp, dummyActDropOff, depPersonId, pid)

            if jointPickUpDropOffFlag:
                # print '\t\t\t\tPerson with no fixed activities found and id
                # is -- ', pid
                self.add_activity_update_depPersonId(
                    [dummyActPickUp, dummyActDropOff], depPersonId, pid)
                return True

        # Person with fixed activities
        # We allocate pickup/dropoff to the first person with fixed activities
        # that we find
        ...
        for pid in self.dailyFixedActPersonIds:
            ...
            if not person._check_for_conflicts_with_activity([dummyActPickUp, dummyActDropOff]):
                ...
                if person._check_for_home_to_home_trips():
                    # print '\t\t\t\tfor the above person there is a
                    # home-to-home conflict'
                    jointPickUpDropOffFlag = False
                    person.remove_episodes([dummyActPickUp, dummyActDropOff])
                else:
                    # print '\t\t\t\tfor the above person there is a overlap
                    # conflict'
                    person.remove_episodes([dummyActPickUp, dummyActDropOff])
                    confActs = person._identify_conflict_activities(
                        [dummyActPickUp, dummyActDropOff])
        ...
        if pid is None:
            self.add_activity_update_depPersonId(
                [dummyActPickUp, dummyActDropOff], depPersonId, 99)
            # print "Exception, --There are no independent adults in the
            # household for hid - %s--" %(self.hid)
            return
    ...
    def check_conflicts_with_pickup_dropoff(self, confActs, dummyActPickUp, dummyActDropOff, depPersonId, pid):
        ...
        if len(confActs) > 2:
            raw_input(
                'More than two conflicts with pickup/dropoff? Not possible?')
        ...
        if len(confActs) == 1:
            ...
            if confAct.startTime <= dummyActPickUp.startTime and confAct.endTime >= dummyActPickUp.endTime:
                ...
                raw_input(
                    'THIS SHOULD NOT HAPPEN THE CONFLICT ACT SHOULD CONFLCIT WITH EITHER PIKCUP OF DROPOFF')
        ...
        if (pickUpConf is not None and dropOffConf is not None):
            if conflictPickup is False or conflictDropoff is False:
                # print '\t\tnot a valid pickup/dropoff because one of the
                # conflicts is not a pickup/dropoff for some other dependent
                # person'
                return False
        ...
        if (conflictPickup or conflictDropoff):
            if conflictPickup:
                ...
                pickUpConf.dependentPersonId = pickUpConf.dependentPersonId * \
                    100. + depPersonId
                if pickUpConf.tripCount < 100:
                    pickUpConf.tripCount = (
                        100 + pickUpConf.tripCount) * 100 + dummyActPickUp.tripCount
                else:
                    pickUpConf.tripCount = pickUpConf.tripCount * \
                        100 + dummyActPickUp.tripCount
            if conflictDropoff:
                #print ("\t\t\t\tconflict is with dropoff")
                # print '\t\t\t\tdropoff act - ', dummyActDropOff
                # print '\t\t\t\tdropoff conflict - ', dropOffConf
                dropOffConf.dependentPersonId = dropOffConf.dependentPersonId * \
                    100. + depPersonId
                if dropOffConf.tripCount < 100:
                    dropOffConf.tripCount = (
                        100 + dropOffConf.tripCount) * 100 + dummyActDropOff.tripCount
                else:
                    dropOffConf.tripCount = dropOffConf.tripCount * \
                        100 + dummyActDropOff.tripCount
            ...
            if pickUpConf is None:
                self.add_activity_update_depPersonId(
                    [dummyActPickUp], pid, depPersonId, dependent=False)
                # we are using the same function for adding and updating
                # deppersonid for indep adult

            if dropOffConf is None:
                self.add_activity_update_depPersonId(
                    [dummyActDropOff], pid, depPersonId, dependent=False)
                # we are using the same function for adding and updating
                # deppersonid for indep adult
    ...
    def allocate_trip_activity_chain(self, depPersonId, actsInTour):
        ...
        for pid in self.noDailyFixedActPersonIds:
            ...
            person = self.persons[pid]
            # self.print_activity_list(person)
            # print 'THE ACT COUNT BEFORE ADDING', person.actCount,
            # len(person.listOfActivityEpisodes)
            ...
            if not person._check_for_conflicts_with_activity(actIncChauffering):
                ...
                # is -- ', pid
                self.add_activity_update_depPersonId(
                    chaufferingEpisodes, depPersonId, pid)
        ...
        if pid is None:
            self.add_activity_update_depPersonId(
                chaufferingEpisodes, depPersonId, 99)
    ...
    def allocate_pickup_dropoff_endact(self, depPersonId, stAct, endAct):
        ...
        for pid in self.noDailyFixedActPersonIds:
            ...
            person.add_episodes(
                [dummyActPickUp, dummyActDropOff, endActToNonDependent], temp=True)
            # print 'THE ACT COUNT AFTER ADDING', person.actCount, len(person.listOfActivityEpisodes)
            # self.print_activity_list(person)
            if not person._check_for_conflicts_with_activity([dummyActPickUp, dummyActDropOff,
                                                              endActToNonDependent]):
                # if not person._check_for_conflicts():
                person.remove_episodes(
        ...
        if not person._check_for_conflicts_with_activity([dummyActPickUp, dummyActDropOff,
            ...
            # print '\t----- NEED TO ADJUST THIS PERSONS ACT SCHEDULE -----'
            # self.print_activity_list(person)
            person.adjust_child_dependencies(
                [dummyActPickUp, dummyActDropOff, endActToNonDependent])
            # self.print_activity_list(person)
        return True
    ...
    def personId_with_least_conflict(self, depPersonId, activityList, personIdList):
        ...
        for pid in personIdList:
            ...
            if person._check_for_home_to_home_trips():
                # print '\t----- CHECKING TO SEE IF THE ADULT WITH LEAST
                # CONFLICT HAS IN_HOME_IN_HOME TRIPS BEING INTRODUCED-----??',
                # pid
                checkForHomeToHomeTrips = True
                # print '\t----- NOT ALLOCATING TO THIS ADULT WITH LEAST
                # CONFLICT BECAUSE IN_HOME_IN_HOME TRIPS ARE BEING
                # INTRODUCED-----', pid
            ...
            # print '\t\t\t\tDependencies - ', checkForDependencies, ',
            # checkForHomeToHomeTrips', checkForHomeToHomeTrips
        ...
        # print personConflictActs
            ...
            # print '\t\t\t\t\tPerson - %s has conflict of duration - %s  for
            # above activity' %(pid, conflict)
        ...
        for pid in personIdsWithConflict:
            # print '\t\t\t\t\tperson - ', pid, 'conflict for person - ',
            # personConflict[pid], 'least conflict', leastConflict
    ...
    def _check_for_dependencies_for_conflictActivities(self, depPersonId, conflictActs, terminal=False):
        # print '\t\t\t\tCHECKING FOR DEPENDENCIES for dep act of person - ',
        # depPersonId
        ...
        for act in conflictActs:
            ...
            if act.dependentPersonId <> 0:  # there is a dependency
                ...
                # if [depPersonId] <>
                # self.parse_personids(act.dependentPersonId) and
                # (act.startTime == 0 or act.endTime == 1439):
                if [depPersonId] <> self.parse_personids(act.dependentPersonId):
    ...
    def personId_with_terminal_episode_overlap(self, depPersonId, activityList, personIdList):
        ...
        for pid in personIdList:
            ...
            conflictWithStartEpisode = person._conflict_duration_with_activity(
                stEpisode)

            enEpisode = person.lastEpisode
            conflictWithEndEpisode = person._conflict_duration_with_activity(
                enEpisode)

            conflictWithTermEpisodes = conflictWithStartEpisode + \
                conflictWithEndEpisode
            conflict = person._conflict_duration()
            person.remove_episodes(activityList)
        ...
        for pid in personConflict.keys():
            ...
            # personConflict[pid], 'least conflict', leastConflict
            if personConflict[pid] == leastConflict:
                # print '\t\t\t\tPerson with least conflict with terminal
                # episode is', pid
                return pid
